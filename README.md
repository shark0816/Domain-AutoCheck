...
<details>
    ...
    2. 新增亮色和暗色主题切换  
    3. 新增 `pp.ua`、`qzz.io`、`dpdns.org`、`xx.kg`、`us.kg` 5 个二级域名自动查询（自动填写注册厂商、注册日期、到期日期、续期链接，默认续期周期 1 年）  
    4. 列表新增分类下拉框过滤，添加域名增加注册账号属性  
</details>

> 现在除了一级域名配合 apikey 查询之外，新增 `pp.ua`、`qzz.io`、`dpdns.org`、`xx.kg`、`us.kg` 这 5 个二级域名时，点击查询后如果不需要填写注册账号和备注，其他信息都会自动填写，可以直接保存。
...
* 支持指定二级域名自动查询：`pp.ua`、`us.kg`、`xx.kg`、`qzz.io`、`dpdns.org`

## ✅二级域名自动查询
支持以下二级域名自动识别注册商、注册日期、到期日期、续期链接，并默认续期周期 1 年：  
`pp.ua` · `us.kg` · `xx.kg` · `qzz.io` · `dpdns.org`
